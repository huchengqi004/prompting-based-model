...
# sam_checkpoint = 'B:\\code\\GrSAM\\data\\work_dir\\SAM-ViT-B\\sam_model_best.pth'

sys.path.append("..")
currentpath = os.path.dirname(os.path.abspath(__file__))
sam_checkpoint = os.path.join(currentpath, 'data\\work_dir\\SAM-ViT-B\\sam_vit_b_01ec64.pth')
# path = 'B:\\code\\GrSAM\\data\\Test\\TA2'
# txt_path = 'B:\\code\\GrSAM\\data\\Test\\label\\F1\\LabelproducedbyGrSAM\\F1-[223, 319].txt'
path = filedialog.askdirectory(title="选择包含待分割的石墨图像所在的文件夹：")    #此处可换成自己存储图像的文件夹路径。
...
for file_name in file_names:
    ...
    if file_name.endswith(".png") or file_name.endswith(".jpg") or file_name.endswith(".tif") or file_name.endswith(".bmp") or file_name.endswith(".TIF"):  # 确保只处理图像文件
        # 图像文件的完整路径
        image_path = os.path.join(path, file_name)
        path_seg = os.path.join(os.path.dirname(path), 'GrSAM-HP', (file_name.split(".")[0] + 'GrSAM-HP' +".png"))
        os.makedirs(os.path.dirname(path_seg), exist_ok=True)

        # 处理单张图像
        process_image(pathroot=path, imagename=file_name, predictor=predictor)
        txt_path_HP = os.path.join(path_seg.split(".")[0] + '-' + str(points[0]) + '.txt')
        # 获取当前时间
        end_time = time.time()
        # 计算并输出代码执行时间
        ...
        # 打开文件以写入模式
        with open(txt_path_HP, 'w') as file:
        ...
        print("时间和提示点框位置的txt文件保留在:", txt_path_HP)
        print("手动分割的图像保留在：", path_seg)
